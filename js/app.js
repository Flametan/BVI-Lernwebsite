...
const NAV = (function(){
  let stack = [];
  const ALL = ['v-home','v-gal','v-gal-organisation','v-gal-brandlehre','v-gal-fahrzeuge','v-gal-einsatz','v-gal-atemschutz','v-gal-beamtenrecht','v-gal-beihilferecht','v-gal-brandbekaempfung','v-gal-einsatztechnik','v-gal-erstehilfe','v-gal-grundlagen','v-gal-fahrzeugnormung','v-gal-fuehrung','v-gal-fwdven','v-gal-gabc','v-gal-geraetepruefung','v-gal-hbkg','v-gal-kartenkunde','v-gal-knoten','v-gal-staatsbuerger','v-gal-th-verkehr','v-gal-leitern','v-gal-uvv','v-gal-waermebildkamera','v-sfs','v-sfs-fwdv3','v-sfs-methodik','v-sfs-rechtsgrundlagen','v-sfs-abc','v-hlfs','v-hlfs-fuehrungsvorgang','v-hlfs-gabc','v-hlfs-vb','v-hlfs-manv','v-hlfs-tunnel','v-hlfs-zugfuehrer','v-hlfs-stab','v-ibk','v-ibk-ta','v-ibk-konflikt','v-ibk-stress','v-ibk-psnv','v-ibk-bgm','v-ibk-pm','v-ibk-zeit','v-vak','v-feuak','v-idf','v-simulator','v-flashcards','v-vorschlaege'];
...
const FLASHCARD_DATA = [
  ...
  {id:'g01',cat:'GAL · Brandlehre',q:'Welche vier Komponenten bilden das Branddreieck/Tetraeder und welche kommt beim Tetraeder hinzu?',a:'<strong>Branddreieck:</strong> Brennstoff, Sauerstoff, Energie (Zuendquelle).<br><strong>Brand-Tetraeder ergaenzt:</strong> ungestoerte Kettenreaktion (Radikalkette). Loescheffekte zielen auf das Entfernen mindestens einer Komponente.'},
  {id:'g02',cat:'GAL · Brandlehre',q:'Nenne die Brandklassen A-F mit jeweils einem Beispielstoff.',a:'<ul><li><strong>A</strong> - feste, glutbildende Stoffe (Holz, Papier)</li><li><strong>B</strong> - fluessige/verfluessigende Stoffe (Benzin, Wachs)</li><li><strong>C</strong> - Gase (Propan, Erdgas)</li><li><strong>D</strong> - Metalle (Mg, Na, Al)</li><li><strong>F</strong> - Speisefette/-oele in Frittiergeraeten</li></ul>'},
  {id:'g03',cat:'GAL · Brandlehre',q:'Was ist ein Flash-Over und welche Vorzeichen warnen davor?',a:'<strong>Flash-Over (Raumdurchzuendung):</strong> alle brennbaren Oberflaechen erreichen Zuendtemperatur und entzuenden sich nahezu gleichzeitig.<br><strong>Vorzeichen:</strong> rollende Rauchwalzen unter der Decke, Pyrolysegasflammen ("Engelshaare"), schnell steigende Deckentemperatur (&gt;500 °C), Pfeifen/Saugen an Oeffnungen.'},
  {id:'g04',cat:'GAL · Brandlehre',q:'Was passiert bei einem Backdraft und wie unterscheidet er sich vom Flash-Over?',a:'<strong>Backdraft (Rauchgasexplosion):</strong> in einem schwelenden, sauerstoffarmen Raum entzuendet sich das fette Rauchgas-Luftgemisch nach ploetzlicher Luftzufuhr (Tueroeffnung) schlagartig.<br><strong>Unterschied:</strong> Flash-Over braucht Sauerstoff und Hitze; Backdraft braucht zusaetzlich die ploetzliche Belueftung eines unterventilierten Raumes.'},
  {id:'g05',cat:'GAL · Brandlehre',q:'Was bedeutet BLEVE und wie entsteht sie?',a:'<strong>BLEVE = Boiling Liquid Expanding Vapour Explosion.</strong><br>Verfluessigtes Gas in einem geschlossenen Behaelter erwaermt sich. Bei Druckverlust verdampft die ueberhitzte Fluessigkeit schlagartig, das Gas-Luftgemisch entzuendet sich. Folge: Druckwelle + Feuerball (50-150 m Radius bei Propan).'},
  {id:'g06',cat:'GAL · Brandlehre',q:'Welche Loeschwirkungen kennst du und ordne Wasser, CO2 und ABC-Pulver zu.',a:'<strong>Wirkungen:</strong> Abkuehlen, Ersticken, Verduennen, Inhibition (Kettenabbruch).<br><ul><li>Wasser → <strong>Abkuehlen</strong></li><li>CO2 → <strong>Ersticken</strong> (Sauerstoffverdraengung)</li><li>ABC-Pulver → <strong>Inhibition</strong> (antikatalytisch)</li></ul>'},
  {id:'g07',cat:'GAL · Brandlehre',q:'Welche drei Mechanismen der Waermeuebertragung sind im Einsatz relevant?',a:'<strong>1. Waermeleitung</strong> (Kondukion) - durch Festkoerper, z. B. Stahltraeger.<br><strong>2. Konvektion</strong> - mit heisser Stroemung, Treppenraum, Rauchgas.<br><strong>3. Waermestrahlung</strong> - elektromagnetisch, ueber freie Strecke, gefaehrlich fuer Nachbargebaeude.'},
  {id:'g08',cat:'GAL · Brandlehre',q:'Nenne die vier Phasen eines Zimmerbrandes.',a:'<strong>1. Entstehungsphase</strong> - lokal, kuehl, sauerstoffreich.<br><strong>2. Wachstumsphase</strong> - rapider Temperaturanstieg, Rauchschicht bildet sich.<br><strong>3. Vollbrandphase</strong> - nach Flash-Over, alle Stoffe brennen, &gt;1000 °C.<br><strong>4. Abklingphase</strong> - Brennstoff verbraucht, Temperatur sinkt.'},
  {id:'g09',cat:'GAL · Atemschutz',q:'Welche Bauarten von Pressluftatmern werden in der Feuerwehr eingesetzt?',a:'<ul><li><strong>1-Flaschen-PA</strong> 6,8 L / 300 bar (Standard)</li><li><strong>2-Flaschen-PA</strong> 2x4 L oder 2x6 L (Langzeit)</li><li><strong>Regenerationsgeraet</strong> (Kreislaufgeraet) fuer Langzeit/Grube</li><li><strong>Druckluftschlauchgeraet</strong> fuer Werks- und Spezialeinsatz</li></ul>'},
  {id:'g10',cat:'GAL · Atemschutz',q:'Wie berechnet man die verfuegbare Atemluft eines PA (Faustformel)?',a:'<strong>Vorrat (L) = Flaschenvolumen × Fuelldruck.</strong><br>Beispiel: 6,8 L × 300 bar = 2.040 L.<br>Abzug: Sicherheitsreserve (~1/3 + Warndruck 55 bar). Einsatzzeit ≈ Vorrat / Atemminutenvolumen (40-80 L/min bei schwerer Arbeit).'},
  {id:'g11',cat:'GAL · Atemschutz',q:'Was regelt die FwDV 7 zum Truppgrundsatz?',a:'<strong>Mindeststaerke:</strong> Trupp (3 Personen: TrFue + 2 TrM).<br><strong>Truppzusammenhalt</strong> durchgehend wahren - keine Trennung im Innenangriff. <strong>Sicherheitstrupp</strong> in gleicher Staerke und Ausruestung steht bereit, ueberwacht durch ASGW.'},
  ...
  {id:'g13',cat:'GAL · Atemschutz',q:'Welche physiologischen Wirkungen haben CO und HCN im Brandrauch?',a:'<strong>CO:</strong> bindet 250x staerker an Haemoglobin als O2 → innere Erstickung; Symptome: Kopfschmerz, Schwindel, kirschrote Haut.<br><strong>HCN (Blausaeure):</strong> blockiert Atmungskette (Zytochrom-Oxidase) → Zellgifterstickung; entsteht bei Polyurethan-, Woll- und Seidenbrand.'},
  {id:'g14',cat:'GAL · Atemschutz',q:'Wann ertoent der Warndruckpfiff und was ist zu tun?',a:'<strong>Warndruck</strong> bei ca. <strong>55 bar</strong> (~1/3 Restvorrat) - akustisches Signal.<br><strong>Reaktion:</strong> sofortiger Rueckzugsbefehl ueber Funk, geordneter Ausstieg im Trupp; Sicherheitstrupp vorinformieren.'},
  {id:'g15',cat:'GAL · Atemschutz',q:'Was ist die G 26 Untersuchung und welche Stufen gibt es?',a:'<strong>G 26 - arbeitsmedizinische Vorsorge fuer Atemschutzgeraetetraeger.</strong><br><ul><li><strong>G 26.1</strong> leichte Filtermasken</li><li><strong>G 26.2</strong> mittelschwere Geraete</li><li><strong>G 26.3</strong> PA, Regenerationsgeraete</li></ul>Gueltigkeit i. d. R. <strong>3 Jahre</strong> (ab 50: 1-2 Jahre).'},
  {id:'g16',cat:'GAL · Atemschutz',q:'Wie funktioniert ein Regenerationsgeraet (Kreislaufgeraet)?',a:'Geschlossenes Atemsystem: ausgeatmete Luft wird in einem <strong>CO2-Absorber</strong> (Natronkalk oder KO2) gereinigt; Sauerstoff aus Druckflasche oder Chemikalpatrone wird nachgespeist. <strong>Einsatzzeit 2-4 h</strong>, ideal fuer Tunnel, U-Bahn, Bergwerk.'},
  {id:'g17',cat:'GAL · Beamtenrecht',q:'Welche Grundpflichten regeln §§ 34-37 BeamtStG?',a:'<ul><li><strong>§ 34</strong> volle Hingabe an den Beruf, achtungs- und vertrauenswuerdiges Verhalten</li><li><strong>§ 35</strong> Folge- und Beratungspflicht gegenueber Vorgesetzten</li><li><strong>§ 36</strong> Verantwortung fuer Rechtmaessigkeit (Remonstration)</li><li><strong>§ 37</strong> Verschwiegenheitspflicht (auch nach Dienstende)</li></ul>'},
  {id:'g18',cat:'GAL · Beamtenrecht',q:'Was besagt das Alimentationsprinzip?',a:'<strong>Hergebrachter Grundsatz</strong> (Art. 33 V GG). Der Dienstherr hat dem Beamten und seiner Familie <strong>lebenslang amtsangemessenen Unterhalt</strong> zu gewaehren (Besoldung, Beihilfe, Versorgung). Dafuer schuldet der Beamte volle Hingabe.'},
  {id:'g19',cat:'GAL · Beamtenrecht',q:'Welche Disziplinarmassnahmen sind nach Disziplinargesetz moeglich?',a:'<strong>Aufsteigend nach Schwere:</strong><ul><li>Verweis</li><li>Geldbusse (bis 1 Monatsgehalt)</li><li>Kuerzung der Dienstbezuege (max. 1/5, bis 3 Jahre)</li><li>Zurueckstufung</li><li>Entfernung aus dem Beamtenverhaeltnis</li><li>Ruhegehaltskuerzung / Aberkennung (Ruhestaendler)</li></ul>'},
  {id:'g20',cat:'GAL · Beamtenrecht',q:'Welche Laufbahngruppen kennt das Beamtenrecht?',a:'<ul><li><strong>Einfacher Dienst</strong> (kaum noch besetzt)</li><li><strong>Mittlerer Dienst</strong> - Brandmeister, A 6-A 9</li><li><strong>Gehobener Dienst</strong> - Brandinspektor bis Brandoberamtsrat, A 9-A 13</li><li><strong>Hoeherer Dienst</strong> - Brandrat aufwaerts, A 13-B</li></ul>'},
  {id:'g21',cat:'GAL · Beamtenrecht',q:'Welche Arten von Beamtenverhaeltnissen gibt es?',a:'<ul><li><strong>Auf Widerruf</strong> - Vorbereitungsdienst</li><li><strong>Auf Probe</strong> - i. d. R. 3 Jahre</li><li><strong>Auf Lebenszeit</strong> - Hauptregelfall</li><li><strong>Auf Zeit</strong> - z. B. Wahlbeamte</li><li><strong>Ehrenbeamter</strong> - ehrenamtliche Funktion</li></ul>'},
  {id:'g22',cat:'GAL · Brandbekaempfung',q:'Welche taktischen Unterschiede gelten zwischen Innen- und Aussenangriff?',a:'<strong>Innenangriff:</strong> PA-Pflicht, HSR/C-Rohr, Truppstaerke, Sicherheitstrupp, defensives Loeschen wegen Flash-Over-Gefahr.<br><strong>Aussenangriff:</strong> kein PA-Zwang, B-Rohre, hoher Wasserdurchsatz, Riegelstellung gegen Brandausbreitung.'},
  {id:'g23',cat:'GAL · Brandbekaempfung',q:'Welche Strahlrohrmodi am Hohlstrahlrohr (HSR) gibt es?',a:'<ul><li><strong>Vollstrahl</strong> - max. Wurfweite</li><li><strong>Spruehstrahl 30-60°</strong> - Brandbekaempfung</li><li><strong>Mannschutzkegel ~90°</strong> - Schutz des Trupps</li></ul>Zusaetzlich variabler Durchfluss (100/200/400 L/min) und Impulsspruehstoss-Technik.'},
  {id:'g24',cat:'GAL · Brandbekaempfung',q:'Welche Voraussetzungen und Vorteile hat die Ueberdruckbelueftung (UDB)?',a:'<strong>Voraussetzungen:</strong> definierte Zu-/Abluftoeffnung, kein Backdraft-Risiko, abgestimmter Loeschangriff.<br><strong>Vorteile:</strong> bessere Sicht, sinkende Temperatur, zuegiger Innenangriff, erleichterte Personenrettung, gezielter Rauchabzug.'},
  {id:'g25',cat:'GAL · Brandbekaempfung',q:'Warum darf ein Fettbrand (Klasse F) nie mit Wasser geloescht werden?',a:'Wasser verdampft im heissen Fett (&gt;300 °C) schlagartig (<strong>1700-fache Volumenzunahme</strong>) → <strong>Fettexplosion</strong> mit Feuerball.<br><strong>Richtig:</strong> Brandklasse-F-Loescher, Deckel auf, Stromzufuhr aus.'},
  {id:'g26',cat:'GAL · Brandbekaempfung',q:'Welche Warnzeichen weisen auf Flash-Over hin?',a:'<ul><li>Rasch fallende Rauchschichtgrenze</li><li>Pyrolysegasflammen unter der Decke (Engelshaare)</li><li>Stark schwarzer, fetter Rauch unter Druck</li><li>Schnell steigende Hitze, schmelzende Kunststoffe</li><li>Pfeifen / Stroemungsgeraeusche</li></ul>'},
  {id:'g27',cat:'GAL · Brandbekaempfung',q:'Welche Warnzeichen deuten auf einen drohenden Backdraft hin?',a:'<ul><li>Gelblich-braeunliche Glasscheiben (Russablagerungen)</li><li>Pulsierende Rauchaustritte ("atmendes" Gebaeude)</li><li>Heisse Tueren ohne sichtbare Flammen</li><li>Pfeifen / Sauggeraeusche an Spalten</li><li>Sehr heisser Rauch unter Druck</li></ul>'},
  {id:'g28',cat:'GAL · Einsatztechnik',q:'Welche Druckschlauchtypen sind in der Feuerwehr genormt?',a:'<ul><li><strong>A 110</strong> mm - Saug-/Hauptleitung</li><li><strong>B 75</strong> mm - Verteilerzulauf, Foerderung</li><li><strong>C 42 / C 52</strong> mm - Angriff</li><li><strong>D 25</strong> mm - Kleinloeschmittel, Waldbrand</li><li><strong>S</strong> - Saugschlauch, formstabil</li></ul>'},
  {id:'g29',cat:'GAL · Einsatztechnik',q:'Faustformel fuer Druckverlust in B-Schlaeuchen.',a:'<strong>~0,1 bar pro 100 m B je 100 L/min Foerderstrom.</strong><br>Beispiel: 800 L/min ueber 200 m B → 0,1 × 8 × 2 ≈ 1,6 bar. Plus Hoehe (1 bar je 10 m) und Strahlrohrdruck (5 bar HSR).'},
  {id:'g30',cat:'GAL · Einsatztechnik',q:'Mit welchem Mundstueckdruck arbeiten gaengige Strahlrohre?',a:'<ul><li>Hohlstrahlrohr <strong>5-7 bar</strong></li><li>CM-Rohr 4-5 bar</li><li>BM-Rohr 5 bar</li><li>Wasserwerfer 8-10 bar</li></ul>Ausgangsdruck Pumpe = Mundstueckdruck + Druckverlust + Hoehendruck.'},
  {id:'g31',cat:'GAL · Einsatztechnik',q:'Welche Schaummittelarten gibt es und wofuer sind sie geeignet?',a:'<ul><li><strong>MBS</strong> - Mehrbereichsschaum, A+B</li><li><strong>AFFF</strong> - Filmbildner, B (Mineraloele)</li><li><strong>AR-AFFF</strong> - alkoholbestaendig, polare Loesungsmittel</li><li><strong>Class A Foam</strong> - feste Stoffe, Waldbrand</li></ul>Verschaeumungszahl: schwer &lt;20, mittel 20-200, leicht &gt;200.'},
  {id:'g32',cat:'GAL · Einsatztechnik',q:'Welche Hydrantenarten gibt es und wie werden sie gekennzeichnet?',a:'<ul><li><strong>Unterflur</strong> - Strassenoberflaeche, Schild "H" auf weiss</li><li><strong>Ueberflur</strong> - Standhydrant mit B-/A-Abgaengen</li><li><strong>Wandhydrant</strong> - Gebaeude (Steigleitung trocken/nass)</li></ul>Schildangaben: Nennweite, Lage, Entfernung in Metern.'},
  {id:'g33',cat:'GAL · Erste Hilfe',q:'Wie lautet das ABCDE-Schema in der praeklinischen Versorgung?',a:'<ul><li><strong>A</strong> Airway (Atemweg/HWS)</li><li><strong>B</strong> Breathing (Atmung)</li><li><strong>C</strong> Circulation (Kreislauf, Blutung stillen)</li><li><strong>D</strong> Disability (Neurostatus, GCS, BZ)</li><li><strong>E</strong> Exposure/Environment (entkleiden, Waermeerhalt)</li></ul>'},
  {id:'g34',cat:'GAL · Erste Hilfe',q:'Wie ist das Verhaeltnis von Herzdruckmassage zu Beatmung bei der CPR Erwachsener?',a:'<strong>30:2</strong> - 30 Kompressionen + 2 Beatmungen. Frequenz <strong>100-120/min</strong>, Tiefe <strong>5-6 cm</strong>, vollstaendige Entlastung. Kind ggf. 15:2 bei zwei Helfern.'},
  {id:'g35',cat:'GAL · Erste Hilfe',q:'Welche Grundschritte gelten beim AED-Einsatz?',a:'<ul><li>Geraet ein, Anweisungen folgen.</li><li>Patient trocknen, Brust entbloessen.</li><li>Elektroden rechts oben sternal / links lateral.</li><li>Analyse - niemand beruehrt den Patienten.</li><li>Schock auf Anweisung, sofort CPR 30:2 weiter.</li></ul>'},
  {id:'g36',cat:'GAL · Erste Hilfe',q:'Welche Schockarten unterscheidet die Notfallmedizin?',a:'<ul><li><strong>Hypovolaemisch</strong> - Volumenverlust</li><li><strong>Kardiogen</strong> - Herzpumpversagen</li><li><strong>Obstruktiv</strong> - Spannungspneumothorax, Lungenembolie</li><li><strong>Distributiv</strong> - septisch, anaphylaktisch, neurogen</li></ul>Symptome: kalte feuchte Haut, Tachykardie, Hypotonie.'},
  {id:'g37',cat:'GAL · Erste Hilfe',q:'Wie berechnet man die verbrannte Koerperoberflaeche nach der 9er-Regel?',a:'<strong>Erwachsener:</strong> Kopf 9 %, je Arm 9 %, je Bein 18 %, Rumpf vorn/hinten je 18 %, Genital 1 %.<br><strong>Kleinflaechen:</strong> Handflaeche ≈ 1 %. <strong>Kritisch:</strong> Erwachsener &gt;15 % KOF, Kind &gt;10 %.'},
  {id:'g38',cat:'GAL · GABC',q:'Was steckt hinter der GAMS-Regel?',a:'<ul><li><strong>G</strong> - Gefahr erkennen</li><li><strong>A</strong> - Absperren</li><li><strong>M</strong> - Menschenrettung</li><li><strong>S</strong> - Spezialkraefte alarmieren (GW-G, Umweltzug, Fachberater)</li></ul>Immer als Erstmassnahme im GABC-Einsatz.'},
  {id:'g39',cat:'GAL · GABC',q:'Welche Bedeutung haben die GHS-Piktogramme GHS01, GHS02 und GHS06?',a:'<ul><li><strong>GHS01</strong> explosionsgefaehrlich (Bombe)</li><li><strong>GHS02</strong> entzuendbar (Flamme)</li><li><strong>GHS06</strong> akut toxisch (Totenkopf)</li></ul>Weitere: GHS03 oxidierend, 04 Druckgas, 05 aetzend, 07 Gesundheit, 08 chronisch (CMR), 09 umweltgefaehrdend.'},
  {id:'g40',cat:'GAL · GABC',q:'Wie sind die ADR-Gefahrgutklassen 1-9 grob gegliedert?',a:'<ul><li>1 Explosive Stoffe</li><li>2 Gase</li><li>3 Entzuendbare Fluessigkeiten</li><li>4 Entzuendbare Feststoffe</li><li>5 Oxidierende / Peroxide</li><li>6 Giftige / ansteckungsgefaehrlich</li><li>7 Radioaktiv</li><li>8 Aetzend</li><li>9 Verschiedene (z. B. Li-Akku)</li></ul>'},
  {id:'g41',cat:'GAL · GABC',q:'Welche Koerperschutzformen kennt der Feuerwehrdienst?',a:'<ul><li><strong>Form 1</strong> Spritzschutz (Einweganzug, Atemschutz)</li><li><strong>Form 2</strong> Kontaminationsschutz</li><li><strong>Form 3 (CSA)</strong> gasdichter Chemikalienschutzanzug + PA innen</li></ul>Auswahl nach Stoff, Konzentration, Expositionszeit, Schutzziel.'},
  {id:'g42',cat:'GAL · GABC',q:'Welche Dekon-Stufen kennt die FwDV 500?',a:'<ul><li><strong>Stufe I - Notdekon</strong> (Wasserspruehnebel, Lebensrettung)</li><li><strong>Stufe II - Standarddekon</strong> (PSA-Reinigung am Dekon-Platz)</li><li><strong>Stufe III - Dekon V</strong> (Verletztendekon mit medizinischer Versorgung)</li></ul>Aufbau am Uebergang Gefahren-/Absperrbereich, immer in Windrichtung "rein-raus".'},
  {id:'g43',cat:'GAL · Fahrzeuge',q:'Welche Kenndaten hat ein HLF 20 nach DIN 14530-27?',a:'<ul><li>Mannschaft <strong>1/8</strong></li><li>Pumpe <strong>FPN 10-2000</strong> (2.000 L/min bei 10 bar)</li><li>Loeschwasser <strong>1.600 L</strong>, Schaummittel 120 L</li><li>4 PA, hydr. Rettungssatz, 13 kVA-Stromerzeuger, Lichtmast, Tauchpumpe</li><li>Zul. Gesamtmasse 16 t</li></ul>'},
  {id:'g44',cat:'GAL · Fahrzeuge',q:'Was bedeutet die Pumpenbezeichnung FPN 10-2000?',a:'<strong>F</strong>euerloesch<strong>p</strong>umpe <strong>N</strong>iederdruck (bis 10 bar), Nennleistung <strong>2.000 L/min bei 10 bar</strong> und 3 m geodaetischer Saughoehe.<br>FPH = Hochdruckpumpe (40 bar). Groessen: 500, 750, 1000, 1500, 2000, 3000, 4000.'},
  {id:'g45',cat:'GAL · Fahrzeuge',q:'Welche Leistungsdaten hat eine DLK 23-12?',a:'<strong>Drehleiter mit Korb, 23 m Nennrettungshoehe bei 12 m Ausladung.</strong><br>Korbnutzlast 270 kg (3 Pers.), Mannschaft 1/2, Werfer 2.500 L/min im Korb. Anleitern bis ca. 8. OG.'},
  {id:'g46',cat:'GAL · HBKG',q:'Welche Aufgaben weist §§ 1-3 HBKG der Gemeinde zu?',a:'<strong>§ 1</strong> Schutz der Bevoelkerung vor Braenden, Unglucksfaellen und Notstaenden.<br><strong>§ 2</strong> Begriffsbestimmungen (Brand-, ABC-, Katastrophenschutz).<br><strong>§ 3</strong> Aufgaben des allgemeinen Brandschutzes - <strong>Pflichtaufgabe der Gemeinde</strong>.'},
  ...
  {id:'g48',cat:'GAL · HBKG',q:'Was regelt § 17 HBKG zur Freistellung von der Arbeit?',a:'FF-Angehoerige sind fuer <strong>Einsatz, Aus- und Fortbildung freizustellen</strong>. Arbeitgeber zahlt Lohn weiter und bekommt die Kosten von der Gemeinde erstattet. Kuendigungsschutz wegen FW-Dienst.'},
  {id:'g49',cat:'GAL · HBKG',q:'Was regelt § 61 HBKG zum Kostenersatz?',a:'Grundsatz: FW-Leistungen sind <strong>unentgeltlich</strong>.<br><strong>Ausnahmen:</strong> vorsaetzlich/grob fahrlaessig herbeigefuehrte Einsaetze, Anlagenbetreiber, boeswillige Falschalarme, TH ausserhalb Brandbekaempfung, BAB-Einsaetze.'},
  {id:'g50',cat:'GAL · Beihilfe',q:'Welche Beihilfebemessungssaetze gelten in Hessen?',a:'<ul><li><strong>Beamter</strong> 50 %</li><li><strong>Beamter mit 2+ Kindern</strong> 70 %</li><li><strong>Ehegatte/Partner</strong> 70 %</li><li><strong>Kinder</strong> 80 %</li><li><strong>Versorgungsempfaenger</strong> 70 %</li></ul>Restkosten ueber private KV (Anwartschaft).'},
  {id:'g51',cat:'GAL · Beihilfe',q:'Welche Aufwendungen sind grundsaetzlich beihilfefaehig?',a:'<strong>Notwendige und wirtschaftlich angemessene Krankheitsaufwendungen:</strong> aerztliche/zahnaerztliche Leistungen (GOAE/GOZ), Arzneimittel, Krankenhaus, Heilmittel, Hilfsmittel, Pflege, Geburt.<br>Nicht beihilfefaehig: Schoenheits-OP, Lifestyle-Praeparate, IGeL ohne med. Indikation.'},
  {id:'g52',cat:'GAL · Beihilfe',q:'Was ist die Kostendaempfungspauschale (KDP) in Hessen?',a:'Jaehrlicher Selbstbehalt nach Besoldungsgruppe (A 7 ~100 €, A 12 ~300 €, B ~750 €). Wird vor Auszahlung der Beihilfe abgezogen. <strong>Sinn:</strong> eigenverantwortliche Kostenkontrolle, Haushaltsentlastung.'},
  {id:'g53',cat:'GAL · Beihilfe',q:'Wo liegt der Unterschied zwischen BBhV und HBeihVO?',a:'<strong>BBhV</strong> - Bundesbeihilfeverordnung, gilt fuer <strong>Bundesbeamte</strong>.<br><strong>HBeihVO</strong> - Hessische Beihilfenverordnung, gilt fuer <strong>Landes-/Kommunalbeamte in Hessen</strong>.<br>Strukturell aehnlich, aber unterschiedliche Saetze, KDP und Ausschlusskataloge.'},
  {id:'g54',cat:'GAL · Kartenkunde',q:'Welchen Massstab hat die TK 25 und was bedeutet das praktisch?',a:'<strong>TK 25 = 1:25.000.</strong> 1 cm Karte = 250 m Natur, 4 cm = 1 km. Standardkarte fuer Feuerwehr, Bundeswehr, Katastrophenschutz. Hoehenlinien 10 m, hohe Detaildichte.'},
  {id:'g55',cat:'GAL · Kartenkunde',q:'Wie liest man eine UTM-Koordinate (z. B. 32U 477892 5544123)?',a:'<ul><li><strong>32U</strong> UTM-Zone (32 = 6°-12° E, U ~48° N)</li><li><strong>477892</strong> Easting (Ostwert, m)</li><li><strong>5544123</strong> Northing (Nordwert vom Aequator, m)</li></ul>Reihenfolge: <strong>"Erst Rechts, dann Hoch"</strong>. Genauigkeit 1 m.'},
  ...
  {id:'g57',cat:'GAL · Knoten',q:'Wofuer wird der doppelte Achtknoten verwendet?',a:'Als <strong>Sicherheitsknoten und Anschlagschlinge</strong> am Seilende: Einbinden von Karabinern, Sicherung von Personen/Geraet. <strong>Vorteil:</strong> leicht zu pruefen, Wirkungsgrad 70-80 %, gut zu loesen.'},
  {id:'g58',cat:'GAL · Knoten',q:'Wofuer dient der Pfahlstich im Feuerwehrdienst?',a:'Als <strong>Rettungsschlinge</strong> - <strong>zieht sich nicht zu</strong>. Doppelter Pfahlstich: Brust-/Hueftschlinge zur Hoehenrettung. Wirkungsgrad ~75 %. Wird zunehmend durch genormte Rettungsgurte ergaenzt.'},
  {id:'g59',cat:'GAL · Knoten',q:'Wann sichert man einen Mastwurf mit halbem Schlag?',a:'Bei <strong>wechselnder Belastung</strong> oder Laengszug auf den Pfosten kann sich der Mastwurf loesen. Deshalb: <strong>Sicherungsstich (halber Schlag)</strong> am losen Ende - z. B. bei Strahlrohrsicherung, am Karabiner oder am Anschlagpunkt.'},
  {id:'g60',cat:'GAL · Geraeteprüfung',q:'Was schreibt die DGUV V 49 zur Pruefung von FW-Geraeten vor?',a:'<strong>DGUV Vorschrift 49 (FW)</strong>: regelmaessige Pruefung aller FW-spezifischen Arbeitsmittel durch <strong>befaehigte Person</strong>. Typische Fristen: PA-Geraet halbjaehrlich + nach Einsatz; Leitern jaehrlich; hydr. Rettungssatz jaehrlich; HSR/Schlaeuche jaehrlich; alle Pruefungen dokumentieren (Pruefkarte/Geraetebuch).'}
...
const PROGRESS = (function(){
  const GROUPS = {
    gal:  ['v-gal-organisation','v-gal-brandlehre','v-gal-fahrzeuge','v-gal-einsatz','v-gal-atemschutz','v-gal-beamtenrecht','v-gal-beihilferecht','v-gal-brandbekaempfung','v-gal-einsatztechnik','v-gal-erstehilfe','v-gal-grundlagen','v-gal-fahrzeugnormung','v-gal-fuehrung','v-gal-fwdven','v-gal-gabc','v-gal-geraetepruefung','v-gal-hbkg','v-gal-kartenkunde','v-gal-knoten','v-gal-staatsbuerger','v-gal-th-verkehr','v-gal-leitern','v-gal-uvv','v-gal-waermebildkamera'],
...
const SEARCH = (function(){
  ...
  const VIEW_LABELS = {
    'v-gal-organisation':'GAL · Organisation','v-gal-brandlehre':'GAL · Brandlehre','v-gal-fahrzeuge':'GAL · Fahrzeuge','v-gal-einsatz':'GAL · Einsatz','v-gal-atemschutz':'GAL · Atemschutz','v-gal-beamtenrecht':'GAL · Beamtenrecht','v-gal-beihilferecht':'GAL · Beihilferecht','v-gal-brandbekaempfung':'GAL · Brandbekämpfung','v-gal-einsatztechnik':'GAL · Einsatztechnik','v-gal-erstehilfe':'GAL · Erste Hilfe','v-gal-grundlagen':'GAL · Naturwiss. Grundlagen','v-gal-fahrzeugnormung':'GAL · Fahrzeugnormung','v-gal-fuehrung':'GAL · Führung','v-gal-fwdven':'GAL · FwDVen','v-gal-gabc':'GAL · G-ABC Lehrgang','v-gal-geraetepruefung':'GAL · Geräteprüfung','v-gal-hbkg':'GAL · HBKG','v-gal-kartenkunde':'GAL · Kartenkunde','v-gal-knoten':'GAL · Knoten & Stiche','v-gal-staatsbuerger':'GAL · Staatsbürgerkunde','v-gal-th-verkehr':'GAL · TH Verkehrsunfall','v-gal-leitern':'GAL · Tragbare Leitern','v-gal-uvv':'GAL · UVV','v-gal-waermebildkamera':'GAL · Wärmebildkamera',
