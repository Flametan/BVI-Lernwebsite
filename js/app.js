...
const FLASHCARD_DATA = [
  ...
  {id:'g19',cat:'GAL · Beamtenrecht',q:'Welche Disziplinarmassnahmen sind nach Disziplinargesetz möglich?',a:'<strong>Aufsteigend nach Schwere:</strong><ul><li>Verweis</li><li>Geldbusse (bis 1 Monatsgehalt)</li><li>Kürzung der Dienstbezüge (max. 1/5, bis 3 Jahre)</li><li>Zurückstufung</li><li>Entfernung aus dem Beamtenverhältnis</li><li>Ruhegehaltskürzung / Aberkennung (Ruheständler)</li></ul>'},
  ...
  {id:'g32',cat:'GAL · Einsatztechnik',q:'Welche Hydrantenarten gibt es und wie werden sie gekennzeichnet?',a:'<ul><li><strong>Unterflur</strong> - Straßenoberfläche, Schild "H" auf weiss</li><li><strong>Überflur</strong> - Standhydrant mit B-/A-Abgängen</li><li><strong>Wandhydrant</strong> - Gebäude (Steigleitung trocken/nass)</li></ul>Schildangaben: Nennweite, Lage, Entfernung in Metern.'},
  ...
  {id:'g38',cat:'GAL · GABC',q:'Was steckt hinter der GAMS-Regel?',a:'<ul><li><strong>G</strong> - Gefahr erkennen</li><li><strong>A</strong> - Absperren</li><li><strong>M</strong> - Menschenrettung</li><li><strong>S</strong> - Spezialkräfte alarmieren (GW-G, Umweltzug, Fachberater)</li></ul>Immer als Erstmassnahme im GABC-Einsatz.'},
  ...
  {id:'g49',cat:'GAL · HBKG',q:'Was regelt § 61 HBKG zum Kostenersatz?',a:'Grundsatz: FW-Leistungen sind <strong>unentgeltlich</strong>.<br><strong>Ausnahmen:</strong> vorsätzlich/grob fahrlässig herbeigeführte Einsätze, Anlagenbetreiber, böswillige Falschalarme, TH ausserhalb Brandbekämpfung, BAB-Einsätze.'},
  ...
  {id:'v04',cat:'VAk · Jur. Denken',q:'Was bedeuten Erschließungsermessen und Auswahlermessen?',a:'<strong>Erschließungsermessen:</strong> Behörde kann entscheiden, ob sie tätig wird (Ausnahme: Ermessensreduzierung auf null bei erheblicher Gefahr).<br><strong>Auswahlermessen:</strong> Behörde wählt aus mehreren geeigneten Mitteln das am wenigsten belastende.<br><strong>Gebundene Entscheidung:</strong> bei „hat"/„muss" – kein Ermessen.'},
  ...
  {id:'f72',cat:'IBK · BGM',q:'Was sind EAP, MBSR und Präsentismus?',a:'<strong>EAP (Employee Assistance Program):</strong> vertrauliches Beratungsangebot für Mitarbeitende bei privaten/beruflichen Problemen.<br><strong>MBSR (Mindfulness-Based Stress Reduction):</strong> achtsamkeitsbasiertes Stressreduktionsprogramm nach Kabat-Zinn (8-Wochen-Kurs).<br><strong>Präsentismus:</strong> Erscheinen am Arbeitsplatz trotz Krankheit – verursacht mehr Produktivitätsverlust als Absentismus.'},
  ...
  {id:'i27',cat:'IdF · Brandschutz',q:'Was ist eine Brandschutzordnung und welche 3 Teile hat sie?',a:'Regelt organisatorischen Brandschutz in Betrieben/Gebäuden (Basis: DIN 14096):<br><strong>Teil A:</strong> Kurzinformation für alle Personen (Aushang, DIN A4, bildhaft).<br><strong>Teil B:</strong> für Beschäftigte – detaillierte Verhaltensregeln + Prävention.<br><strong>Teil C:</strong> für Mitarbeitende mit Sonderaufgaben (Brandschutzhelfer, Evakuierungshelfer).'},
  ...
  {id:'g78',cat:'GAL · Leitern',q:'Welche Leiterarten kennt die Feuerwehr und was sind ihre Einsatzgrenzen?',a:'<strong>Steckleiter (StL4):</strong> 4 Elemente à 2,65 m, Rettungshöhe ca. 8 m – tragbar, vielseitig.<br><strong>Schiebleiter:</strong> bis 9 m Rettungshöhe; schiebbar.<br><strong>Hakenleiter:</strong> Einhängen an Fensterbänken, für Stockwerkverbindung.<br><strong>Drehleiter (DLK 23-12):</strong> bis 23 m Nennrettungshöhe, motorisiert.<br>Prüfung nach DGUV V 49 jährlich.'},
...
/* ======================================================================
   AURORA – Canvas 2D breathing wave background (BVI palette)
====================================================================== */
(function(){
  if(window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

  const c1 = document.getElementById('aurora-canvas');
  const c2 = document.getElementById('aurora-canvas-2');
  if(!c1 || !c2) return;
  ...
  const PAL=[
    ...
    {r:160,g:130,b:50},  // dark gold
    {r:139,g:30, b:30},  // #8B1A1A red
    {r:21, g:43, b:82},  // #152B52 navy
    {r:220,g:185,b:100}, // light gold
    {r:100,g:60, b:10},  // deep amber
  ];

  let mouseX=.5, mouseY=.5;
  document.addEventListener('mousemove', e=>{ mouseX=e.clientX/innerWidth; mouseY=e.clientY/innerHeight; });

  function drawWaves(t, spd){
    const ww=innerWidth, hh=innerHeight;
    g1.clearRect(0,0,ww,hh);
    g1.fillStyle='#080F1C';
    g1.fillRect(0,0,ww,hh);

    const breathe=Math.sin(t*.4*spd)*.3+.7;

    for(let L=0; L<5; L++){
      const lr=L/5, col=PAL[L%PAL.length];
      const alpha=(0.09+lr*.07)*breathe;
      const baseY=hh*(0.3+lr*.4), amp=(60+L*30)*breathe;
      const freq=0.003-L*.0003, speed=t*(0.3+L*.1)*spd;
      const rotAng=t*.05*spd+L*.5;
      const cx=ww*.5+Math.sin(t*.2*spd+L)*ww*.15;

      g1.beginPath();
      for(let x=-20; x<=ww+20; x+=3){
        const env=Math.exp(-((x-cx)/(ww*.5))**2*.8);
        const mi=Math.exp(-(x/ww-mouseX)**2*8)*30*(mouseY-.5);
        const y=baseY
          +Math.sin(x*freq+speed)*amp*env
          +Math.sin(x*freq*1.5+speed*.7+rotAng)*amp*.5*env
          +Math.cos(x*freq*.5+speed*1.3)*amp*.3*env+mi;
        x===-20 ? g1.moveTo(x,y) : g1.lineTo(x,y);
      }
      g1.lineTo(ww+20,hh+20); g1.lineTo(-20,hh+20); g1.closePath();

      const gr=g1.createLinearGradient(0,baseY-amp,0,hh);
    ...
    const ccx=ww*.55+Math.sin(t*.1*spd)*50, ccy=hh*.45+Math.cos(t*.12*spd)*30;
    const maxR=Math.min(ww,hh)*.45;

    for(let ring=0; ring<3; ring++){
      const rr=maxR*(.5+ring*.2), ra=(.04-ring*.01)*breathe, col=PAL[ring+1];
      g1.beginPath();
      for(let a=0; a<Math.PI*2; a+=.02){
        const wo=Math.sin(a*6+t*.8*spd+ring*2)*20*breathe
                +Math.cos(a*3-t*.5*spd+ring)*15*breathe;
        const px=ccx+Math.cos(a+t*.05*spd*(ring+1))*(rr+wo);
        const py=ccy+Math.sin(a+t*.05*spd*(ring+1))*(rr+wo);
    ...
    gg.addColorStop(.3, `rgba(160,130,50,${.03*breathe})`);
    gg.addColorStop(1,  'rgba(0,0,0,0)');
    g1.fillStyle=gg; g1.fillRect(0,0,ww,hh);
  }

  function drawWire(t, spd){
    const ww=innerWidth, hh=innerHeight;
    g2.clearRect(0,0,ww,hh);
    const breathe=Math.sin(t*.4*spd)*.3+.7;
    for(let i=0; i<18; i++){
      const ratio=i/18, baseY=hh*(.15+ratio*.7);
      const alpha=.06+Math.sin(ratio*Math.PI)*.08;
      g2.beginPath();
      g2.strokeStyle=`rgba(201,168,76,${alpha*breathe})`;
      g2.lineWidth=.5;
      for(let x=0; x<=ww; x+=4){
        const amp=(25+ratio*40)*breathe, speed=t*(.5+ratio*.3)*spd;
        const y=baseY
          +Math.sin(x*(.004+ratio*.001)+speed)*amp
          +Math.cos(x*(.004+ratio*.001)*2.1-speed*.6)*amp*.3;
