...
const PROGRESS = (function(){
  ...
  function updateUI(){
    ...
    ['gal','sfs','hlfs','ibk','vak','feuak','idf'].forEach(g => {
      ...
      const t = views.length;
      const el = document.getElementById('prog-'+g);
      if(!el) return;
      const pct = t ? Math.round(d/t*100) : 0;
      el.innerHTML = '<span class="tp-fill" style="width:'+pct+'%"></span><span class="tp-num">'+d+'/'+t+'</span>';
...
const QUIZ=(function(){
  ...
  return {
    init(){
      if(!Q){
        const il=document.getElementById('quiz-learn-inner');
        if(il)il.innerHTML='<div style="padding:2rem;text-align:center;color:var(--c-slate)">Lade Quiz-Daten…</div>';
        fetch('./js/quiz-data.json').then(r=>r.json()).then(data=>{Q=data;QUIZ.init();})
          .catch(()=>{const il2=document.getElementById('quiz-learn-inner');if(il2)il2.textContent='Fragen konnten nicht geladen werden.';});
        return;
      }
